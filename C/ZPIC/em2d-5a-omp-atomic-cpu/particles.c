...
#pragma omp parallel default(none)                                \
    shared(current_Jx, current_Jy, current_Jz,  spec_ix, spec_iy, \
           spec_ux, spec_uy, spec_uz, spec_x, spec_y)             \
    private(di, dj, dx, dy, qvz, x1, y1)
  {
  #pragma omp for private(di, dj, dx, dy, qvz, x1, y1) schedule(auto)
  for (int i = 0; i < spec_np; i++) {
    // Load particle momenta
