...
void spec_advance_outlining(
    ...
    t_fld *restrict Bz,
    // Current data
    const int current_size, const int current_nrow, t_fld *restrict current_Jx, t_fld *restrict current_Jy,
  ...
  int Jsize = current_size;
#pragma omp target teams distribute parallel for schedule(auto)                \
            private(di, dj, dx, dy, qvz, x1, y1)                               \
	    shared(spec_ux, spec_uy, spec_uz)                                  \
	    map(to: dt_dx, dt_dy, qnx, qny, spec_np, spec_q, current_nrow,     \
             spec_ux[0:spec_np-1], spec_uy[0:spec_np-1], spec_uz[0:spec_np-1]) \
            map(tofrom: current_Jx[0:Jsize-1], current_Jy[0:Jsize-1],          \
             current_Jz[0:Jsize-1], spec_ix[0:spec_np-1],                      \
	     spec_iy[0:spec_np-1], spec_x[0:spec_np-1], spec_y[0:spec_np-1]) 
  for (int i = 0; i < spec_np; i++) {
    // Load particle momenta
...
void spec_advance(t_species *spec, t_emf *emf, t_current *current,
  ...
  spec_advance_outlining( // spec data
      ...
      // current data
      current->size, current->nrow, data->current_data.Jx, data->current_data.Jy,
