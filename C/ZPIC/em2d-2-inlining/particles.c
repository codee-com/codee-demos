...
void spec_advance_outlining(
  ...
  for (int i = 0; i < spec_np; i++) {
    ...
    for (k = 0; k < vnp; k++) {
      ...
      current_Jx[vp[k].ix + nrow * vp[k].iy] += wl1 * wp1[0];
      current_Jx[vp[k].ix + nrow * (vp[k].iy + 1)] += wl1 * wp1[1];

      current_Jy[vp[k].ix + nrow * vp[k].iy] += wl2 * wp2[0];
      current_Jy[vp[k].ix + 1 + nrow * vp[k].iy] += wl2 * wp2[1];
