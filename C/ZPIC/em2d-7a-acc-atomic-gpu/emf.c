...
{   
	...
	emf->B = emf->B_buf + gc[0][0] + gc[1][0] * emf->nrow;
        ...
        emf->offset = gc[0][0] + gc[1][0] * emf->nrow;
...
void emf_report( const t_emf *emf, const char field, const char fc )
{
    ...
    axis[0].min = 0.0;
    axis[0].max = emf->box[0];
    axis[0].label = "x_1";
    axis[0].units = "c/\\omega_p";

    axis[1].min = 0.0;
    axis[1].max = emf->box[1];
    axis[1].label = "x_2";
