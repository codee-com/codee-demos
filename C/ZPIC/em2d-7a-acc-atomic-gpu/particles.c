...
void spec_advance_outlining(
    ...
    t_fld *restrict Bz,
    // Current data
    const int current_size, const int current_nrow, t_fld *restrict current_Jx,
    t_fld *restrict current_Jy, t_fld *restrict current_Jz) {
  t_part_data qvz;
  t_part_data x1, y1;
...
#pragma acc data copyin(dt_dx, dt_dy, qnx, qny, spec_np, spec_q,               \
                        spec_ux [0:spec_np-1], spec_uy [0:spec_np-1],          \
                        spec_uz [0:spec_np-1], current_nrow)                   \
  copy(current_Jx [0:Jsize-1], current_Jy [0:Jsize-1], current_Jz [0:Jsize-1], \
       spec_ix [0:spec_np-1], spec_iy [0:spec_np-1], spec_x [0:spec_np-1],     \
       spec_y [0:spec_np-1])
  {
  #pragma acc parallel
  {
...
void spec_advance(t_species *spec, t_emf *emf, t_current *current,
  ...
  spec_advance_outlining( // spec data
      ...
      // current data
      current->size, current->nrow, data->current_data.Jx,
      data->current_data.Jy, data->current_data.Jz);
